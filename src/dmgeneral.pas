...
uses
  ...
  //,fpspreadsheet, fpsallformats, laz_fpspreadsheet
    , RLReport, RLPDFFilter, RLXLSFilter, RLXLSXFilter, RLHTMLFilter, RLParser
  ;
...
const
  ...
  _PRN_RECIBOS_INTERNO_ = 'recinterno.lrf';
 ...
 // usado principalmente en listado_CC
  MAX_INCLUIR = 5; //Cantidad de items a incluir (Compras, ventas, OP, Pedidos)
  INC_COMPRAS = 0;
  INC_VENTAS = 1;
  INC_OP = 2;
  INC_PEDIDOS = 3;
...
type
  ...
  public
    ...
    procedure CargarComboBoxGUID (var elComboBox: TComboBox; campoVisible, campoIndice: string; var tablaRelleno: TZQuery);
    ...
    procedure AbrirTugsQuerys (var elDM: TDataModule);
...
procedure TDM_General.AbrirConexion;
var
 base: string;
 host: string;
 libr: string;
...
procedure TDM_General.CargarComboBoxGUID(var elComboBox: TComboBox; campoVisible,
  campoIndice: string; var tablaRelleno: TZQuery);
var
 r: TIDGuidCombo;
begin
  ...
  begin
    ...
    While Not EOF do
    begin
      r:= TIDGuidCombo.Create;
      r.dato:= FieldByName(campoVisible).asString;
      r.id:= FieldByName (campoIndice).AsString;
      elComboBox.Items.AddObject(r.dato, TObject(r));
      Next;
    end;
  end;
  if elComboBox.Items.Count > 0 then
...
begin
  if (elComboBox.Items.Count > 0)  and (elComboBox.ItemIndex >= 0)then
  begin
    r:= TIDGuidCombo(elComboBox.Items.Objects[elComboBox.ItemIndex]);
    Result:= r.id;
  end
  else
    Result:= GUIDNULO;
...
begin
  ...
  begin
    ...
    begin
      (elDM.Components[i] as TZQuery).Connection:= cnxBase ;
    end;
  end;
...
procedure TDM_General.AbrirTugsQuerys(var elDM: TDataModule);
var
 i: integer;
begin
  for i:= 0 to elDM.ComponentCount - 1 do
  begin
    if (elDM.Components[i] is TZQuery) and (upcase (copy((elDM.Components[i] as TZQuery).Name,1,3)) = 'TUG') then
    begin
      with (elDM.Components[i] as TZQuery) do
      begin
        if active then close;
        Open;
      end;
...
(*
procedure TDM_General.ExportarXLS(laTabla: TRxMemoryData; elArchivo, laHoja: string);


  procedure TitulosXLS(var laHoja: TsWorksheet;
    ...
    begin
      if (((Copy (UpperCase(TRIM(laTabla.Fields[idx].FieldName)),1,2)) <> 'ID') and
         ((Copy (UpperCase(TRIM(laTabla.Fields[idx].FieldName)),1,3)) <> 'REF')
          and((Copy (UpperCase(TRIM(laTabla.Fields[idx].FieldName)),Length(laTabla.Fields[idx].FieldName)-2,3)) <> '_ID')
          and(UpperCase(TRIM(laTabla.Fields[idx].FieldName)) <> 'BVISIBLE')
          )
           then
      ...
      begin
        ...
        begin
         if (
             ...
             and((Copy (UpperCase(TRIM(Fields[campos].FieldName)),Length(Fields[campos].FieldName)-2,3)) <> '_ID')
             and(UpperCase(TRIM(Fields[campos].FieldName)) <> 'BVISIBLE')
            )
            then
         begin
          ...
          begin
            ...
            if Fields[campos].DataType in [ftDate,  ftTime, ftDateTime] then
              ...
              laWorksheet.WriteUTF8Text(idx + 1, campos, (FieldByName(Fields[campos].FieldName).asString));
         end;
        end;
        Next;
      end;
